<section class='about'>
  ...
  <article class='biography'>
    ...
    <section class='about-biography'>
      <p>
        Je travaille dans le web depuis bientôt {{ $work }} ans au {{ HTML::blank_link('http://www.stappler.fr/', 'Principe de Stappler') }}.
      ...
      <p>
        Je suis sérieux, passioné, et au courant des derniers mouvements d'un domaine changeant au jour-le-jour. Ce via les nombreux designers et developpeurs influents que je suis activement sur des réseaux sociaux comme {{ HTML::blank_link('http://twitter.com/Anahkiasen', 'Twitter') }}.<br />
        ...
        <article class='social'>
          ...
          <ul>
            @foreach($social as $service)
              <li>
                {{ HTML::image('app/svg/'.$service->icon, $service->name) }} {{ HTML::blank_link($service->link, $service->name) }}
              </li>
            @endforeach
          </ul>
        </article>
